...
const Navbar = ({ user, setsearchTerm, searchTerm }) => {
  ...
  const handleChange = (e) => {
    if (e.target.value.length > 20) {
      setError("Your title cant be more than 20 characters");
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:8080/getdetail/${userr ? userr.username : "guest"}`
      )
      .then((response) => {
